...
class MixtureDistribution:
    ...
    # 从混合分布中取样，从列表中的正态分布取样的概率与正态分布的样本数成正比# 优化方法是预先计算每个分布的累积分布函数（CDF），然后使用二分查找来高效地从混合分布中取样

    def sample(self, sample_num):
        samples = []
        for i in range(sample_num):

            dist = random.choice(self.distribution_list)

            if isinstance(dist, MixtureDistribution):
                samples.append(dist.sample(sample_num))
            else:
